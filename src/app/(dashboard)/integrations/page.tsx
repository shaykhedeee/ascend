      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-5 border border-zinc-900 bg-zinc-950">
          <div className="flex items-center gap-2 border-b border-zinc-900 px-5 py-2">
            ...
            <span className="font-mono text-[9px] tracking-widest text-orange-600">DEVELOPER :: INTEGRATIONS_HUB</span>
          </div>
          ...
          <div className="flex border-t border-zinc-900">
            {[
              ...
              <button key={id} onClick={() => setTab(id)}
                className={cn('flex flex-1 items-center justify-center gap-1.5 border-b-2 py-2.5 font-mono text-[9px] tracking-widest transition',
          ...
          <div className="space-y-4">
              ...
              <div className="border border-zinc-900 bg-zinc-950">
                ...
                <form onSubmit={handleCreateWebhook} className="p-4 space-y-3">
                  ...
                  <input value={webhookUrl} onChange={(e) => setWebhookUrl(e.target.value)} placeholder="Endpoint URL (https://...)" required
                    className="h-9 w-full border border-zinc-800 bg-black px-3 font-mono text-xs text-zinc-200 placeholder:text-zinc-400 focus:border-orange-800 focus:outline-none" />
                  <div>
                    <p className="mb-2 font-mono text-[9px] tracking-widest text-zinc-400">SELECT_EVENTS ({selectedEvents.length} selected)</p>
                ...
                {webhooks.map((wh: any) => (
                  <div key={wh._id} className="flex items-start gap-3 p-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-mono text-[10px] text-zinc-500 truncate">{wh.url}</p>
                      <div className="mt-1 flex flex-wrap gap-1">
                        {wh.events.map((evt: string) => (
                          <span key={evt} className="border border-zinc-800 px-1.5 font-mono text-[8px] text-zinc-400">{evt}</span>
                        ))}
                      </div>
                    </div>
                    <div className="flex gap-1.5 shrink-0">
                      <button onClick={() => toggleWebhook({ id: wh._id })}
                        className={cn('border px-2.5 py-1 font-mono text-[8px] tracking-widest transition',
            ...
            {webhooks?.length === 0 && !showWebhookForm && (
              <div className="border border-zinc-900 bg-zinc-950 py-10 text-center">
                ...
                <p className="font-mono text-[10px] tracking-widest text-zinc-400">NO_WEBHOOKS_CONFIGURED</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {newKey && (
              <div className="border border-green-900 bg-green-950/10 p-4">
                <p className="mb-2 font-mono text-[9px] tracking-widest text-green-500">KEY_GENERATED — COPY_NOW_NOT_SHOWN_AGAIN</p>
                ...
                <button onClick={() => setNewKey(null)} className="mt-2 font-mono text-[9px] text-zinc-400 transition hover:text-zinc-400">[DISMISS]</button>
              </div>
            )}
            ...
            <div className="border border-zinc-900 bg-zinc-950">
              ...
              <form onSubmit={handleGenerateKey} className="flex gap-3 p-4">
                ...
                <button type="submit" disabled={generatingKey || !keyName}
                  className="flex items-center gap-1.5 border border-orange-800 bg-orange-950/30 px-5 py-2 font-mono text-[10px] tracking-widest text-orange-500 transition hover:bg-orange-950/60 disabled:opacity-40">
                ...
                {apiKeys.map((key: any) => (
                  <div key={key._id} className="flex items-center gap-3 p-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="font-mono text-[9px] text-zinc-400">{key.keyPrefix}••••••••••••••••••••••••</p>
                      {key.lastUsedAt && (
                        <p className="font-mono text-[8px] text-zinc-400">Last used: {new Date(key.lastUsedAt).toLocaleDateString()}</p>
                      )}
                    </div>
                    <button onClick={() => revokeKey({ id: key._id })}
                      className="border border-red-900 px-2.5 py-1 font-mono text-[9px] text-red-600 transition hover:bg-red-950/20">
                      [REVOKE]
                    </button>
                  </div>
                ))}
              ...
              <div className="p-4 space-y-2">
                {[
                  ...
                  <div key={endpoint} className="flex items-center gap-3">
                    <span className={cn('w-10 text-center border font-mono text-[8px] py-0.5 shrink-0',
                      method === 'GET' ? 'border-green-900 text-green-600' : 'border-blue-900 text-blue-600'
                    )}>{method}</span>
                    <code className="font-mono text-[10px] text-zinc-400">{endpoint}</code>
                    <span className="font-mono text-[9px] text-zinc-400">{desc}</span>
                  </div>
                ))}
              </div>
            </div>
